...
async def handle_surname(msg: types.Message):
    ...
    row = df[df["surname"].str.lower() == surname]
    ...
    if not zone:
        await msg.answer("❌ Не зміг визначити зону за станцією")
        return
    ...
    await msg.answer(f"✅ {person['surname']} додано в зону {zone}")
